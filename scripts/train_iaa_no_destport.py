...
print(f"Device: {device} — IAA without Destination Port")
...
torch.manual_seed(SEED)
model = IAATransformer(input_size=input_size, d_model=128, nhead=4, num_layers=3, num_classes=15).to(device)
...
torch.save(model.state_dict(), '/scratch/kdhungel/iads-project/models/iaa_no_destport.pth')
...
with open('/scratch/kdhungel/iads-project/results/iaa_no_destport_report.txt', 'w') as f:
