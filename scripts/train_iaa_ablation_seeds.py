...
print(f"Device: {device} — IAA Ablation (alpha=0), Seed: {SEED}")
...
class IAATransformer(nn.Module):
    def __init__(self, input_size, d_model, nhead, num_layers, num_classes, dropout=0.3):
        ...
        self.dropout = nn.Dropout(dropout)
        # Alpha FIXED at 0 — ablation study
    ...
    def forward(self, x, freq_bias_all):
        ...
        x = self.dropout(x)
        x = self.fc(x)
        x = x + torch.abs(self.alpha) * freq_bias_all  # always 0
        return x
...
torch.save(model.state_dict(), f'/scratch/kdhungel/iads-project/models/iaa_ablation_seed{SEED}.pth')
print("Model saved.")
...
with open(f'/scratch/kdhungel/iads-project/results/iaa_ablation_seed{SEED}_report.txt', 'w') as f:
