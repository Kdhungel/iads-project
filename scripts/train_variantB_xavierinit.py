...
print(f"Device: {device} — Variant B: xavier init, alpha=0, Seed: {SEED}")
...
class IAAAttention(nn.Module):
    def __init__(self, d_model, nhead, attn_dropout=0.3):
        ...
        # VARIANT B: xavier init matching nn.MultiheadAttention._reset_parameters
        ...
        nn.init.constant_(self.W_q.bias, 0.); nn.init.constant_(self.W_k.bias, 0.); nn.init.constant_(self.W_v.bias, 0.); nn.init.constant_(self.W_o.bias, 0.)
    def forward(self, x):
...
class IAATransformerLayer(nn.Module):
    def __init__(self, d_model, nhead, dim_feedforward=512, dropout=0.3):
        ...
        self.attention = IAAAttention(d_model, nhead, attn_dropout=dropout)
...
torch.save(model.state_dict(), f'/scratch/kdhungel/iads-project/models/variantB_seed{SEED}.pth')
print("Model saved.")
...
with open(f'/scratch/kdhungel/iads-project/results/variantB_seed{SEED}_report.txt', 'w') as f:
