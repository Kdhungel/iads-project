...
ATTN_DROPOUT = float(sys.argv[2]) if len(sys.argv) > 2 else 0.0
random.seed(SEED)
np.random.seed(SEED)
...
print(f"Device: {device} — Dropout sweep: attn_dropout={ATTN_DROPOUT}, alpha=0, Seed: {SEED}")
...
class IAAAttention(nn.Module):
    def __init__(self, d_model, nhead, attn_dropout=0.3):
        ...
        # VARIANT A: dropout on attention weights, matching nn.MultiheadAttention
...
class IAATransformerLayer(nn.Module):
    def __init__(self, d_model, nhead, dim_feedforward=512, dropout=0.3):
        ...
        self.attention = IAAAttention(d_model, nhead, attn_dropout=ATTN_DROPOUT)
...
torch.save(model.state_dict(), f'/scratch/kdhungel/iads-project/models/sweep_d{ATTN_DROPOUT}_seed{SEED}.pth')
print("Model saved.")
...
with open(f'/scratch/kdhungel/iads-project/results/sweep_d{ATTN_DROPOUT}_seed{SEED}_report.txt', 'w') as f:
