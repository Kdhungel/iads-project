...
print(f"Device: {device} — Transformer without Destination Port")
...
if 'Destination Port' in X_train_raw.columns:
    X_train_raw = X_train_raw.drop(columns=['Destination Port'])
    X_val_raw = X_val_raw.drop(columns=['Destination Port'])
    print(f"Destination Port removed. Features: {X_train_raw.shape[1]}")
...
train_loader = DataLoader(NetworkFlowDataset(X_train_resampled, y_train_resampled), batch_size=512, shuffle=True)
...
torch.manual_seed(SEED)
model = TransformerClassifier(input_size=input_size, d_model=128, nhead=4, num_layers=3, num_classes=15).to(device)
...
torch.save(model.state_dict(), '/scratch/kdhungel/iads-project/models/transformer_no_destport.pth')
print("Model saved.")
...
with open('/scratch/kdhungel/iads-project/results/transformer_no_destport_report.txt', 'w') as f:
